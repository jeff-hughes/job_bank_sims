...
We can feed the data, sample a context, its reward for a specific action, and also the optimal action for a given context.
"""
...
def run_contextual_bandit(context_dim, num_actions, dataset, algos):
    ...
    Returns:
        ...
        times: Time to train each algorithm (size num_algorithms).
    """
    ...
    h_actions = np.empty((num_contexts, len(algos)), float)
    h_rewards = np.empty((num_contexts, len(algos)), float)
    times = np.empty(len(algos), float)

    # Run the contextual bandit process
    for i, a in enumerate(algos):
        print(f"Training algorithm {a.name}...")
        t_init = time.time()

        actions = np.empty(num_contexts, float)
        rewards = np.empty(num_contexts, float)
        ...
        h_actions[:, i] = actions
        h_rewards[:, i] = rewards
        times[i] = time.time() - t_init

    return h_actions, h_rewards, times


class ContextualBandit(object):
    """Implements a Contextual Bandit with d-dimensional contexts and
    k arms."""
    ...
    def feed_data(self, data):
        ...
        Args:
        data: Numpy array with shape [n, d+k], where n is the number
        of contexts, d is the dimension of each context, and k the
        number of arms (rewards).

        Raises:
        ValueError: when data dimensions do not correspond to the
        object values.
        """
